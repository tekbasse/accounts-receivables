...
-- SL has a version of this already
...
--   	billing_address 	integer references qal_ec_addresses(address_id),
  ...
  -- gift certificates are products with serial numbers and temporary value accounts, so differentiation to separate tables is not
  -- necessary. The general case should cover gift certificates, credits, gift cards etc..
...
--           -- queries)
--           amount_remaining_p      boolean default '1',
...
--           tax_exempt_p            boolean default '0',
...
--        authorized_date         timestamptz,
--        -- set when to_be_captured_p becomes '1'; used in cron jobs
...
--where (transaction_type='charge' and to_be_captured_p='1' and failed_p='0')
--or (transaction_type='refund' and failed_p='0');
...
--	
--
--        IF coalesce(taxes.shipping_p,''0'') = ''0'' THEN
...
--        SET amount_remaining_p = case when amount > bal_amount_used then ''1'' else ''0'' end
